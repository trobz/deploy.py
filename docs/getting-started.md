...
Install `deploy` with [`uv`](https://docs.astral.sh/uv/) (recommended):
...
```bash
pip install https://github.com/trobz/deploy.py.git
```

## Remote host requirements

The following tools must be pre-installed on the **target host** before using `deploy`:
